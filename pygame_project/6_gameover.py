...
balls.append({
  ...
  "to_x": 3, #공의 x축 이동방향, -3은 왼쪽방향
  "to_y": -6, #공의 y축 이동방향
  "init_spd_y": ball_speed_y[0] #스테이지에서 튕겼을 때의 y 의 최초속도
})


#사라질 무기, 공 정보를 저장하는 변수
weapon_to_remove = -1
ball_to_remove = -1
...
while running:
  ...
  for ball_idx, ball_val in enumerate(balls):
      ...
      #가로벽에 닿았을 때 공 위치 변경(튕겨나오는 효과)
      if ball_pos_x < 0 or ball_pos_x > screen_width - ball_width:
        ball_val["to_x"] = ball_val["to_x"] * -1
    
      #세로위치, 스테이지에 튕겨서 올라가는 처리
  ...
  for ball_idx, ball_val in enumerate(balls):
      ...
      for weapon_idx, weapon_val in enumerate(weapons):
          ...
          if weapon_rect.colliderect(ball_rect):
              ...
              if ball_img_idx < 3: #가장 작은 공이 아니라면 다음 단계의 공으로 쪼개짐
                #현재 공 크기 정보를 가지고 옴
                ...
                balls.append({
                  ...
                  "pos_y": ball_pos_y + (ball_hieght/2) - (small_ball_height/2), #공의 y좌표
